...
namespace ABPEcommerce.Seeding
{
    public class IdentityDataSeeder : ITransientDependency, IIdentityDataSeeder
    {
        ...
        public IdentityDataSeeder(IGuidGenerator guidGenerator, IIdentityRoleRepository roleRepository, IIdentityUserRepository userRepository,
                                  ILookupNormalizer lookupNormalizer, IdentityUserManager userManager, IdentityRoleManager roleManager,
                                  ICurrentTenant currentTenant, IOptions<IdentityOptions> identityOptions)
        {
        ...
        public virtual async Task<IdentityDataSeedResult> SeedAsync(string adminEmail, string adminPassword, Guid? tenantId = null)
        {
            using (CurrentTenant.Change(tenantId))
            {
                ...
                adminUser = new IdentityUser(GuidGenerator.Create(), adminEmail, adminEmail, tenantId)
                {
                    Name = "Admin"
                };
